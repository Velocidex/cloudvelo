...
Override these VQL functions. Velociraptor uses the fact that result
sets are stored in files to quickly copy the files into a zip archive
but we need to export the data from opensearch into JSON files first.
...
func (self *CreateFlowDownload) Call(ctx context.Context,
	...
	arg := &downloads.CreateFlowDownloadArgs{}
	...
	switch arg.Type {
	case "report":
		result, err := downloads.CreateFlowReport(
...
func (self CreateFlowDownload) Info(scope vfilter.Scope, type_map *vfilter.TypeMap) *vfilter.FunctionInfo {
	return &vfilter.FunctionInfo{
		...
		ArgType: type_map.AddType(scope, &downloads.CreateFlowDownloadArgs{}),
	}
}
...
func (self *CreateHuntDownload) Call(ctx context.Context,
	...
	arg := &downloads.CreateHuntDownloadArgs{}
	err := arg_parser.ExtractArgsWithContext(ctx, scope, args, arg)
	if err != nil {
...
func (self CreateHuntDownload) Info(scope vfilter.Scope, type_map *vfilter.TypeMap) *vfilter.FunctionInfo {
	return &vfilter.FunctionInfo{
		...
		ArgType: type_map.AddType(scope, &downloads.CreateHuntDownloadArgs{}),
	}
}
...
func downloadFlowToZip(
	...
	err = writeJSONLResultSetToFile(ctx, config_obj, client_id, hostname,
		password, zip_writer, flow_path_manager.Log())
	if err != nil {
		return err
	}
	...
	for _, artifact_with_results := range flow_details.Context.ArtifactsWithResults {
		...
		err = writeJSONLResultSetToFile(ctx, config_obj,
			client_id, hostname, password, zip_writer, path_manager.Path())
		if err != nil {
			continue
		}
		...
		if write_csv {
			err = writeCSVResultSetToFile(ctx, config_obj,
				client_id, hostname, password, zip_writer, path_manager.Path())
			if err != nil {
				continue
			}
	...
	for row := range reader.Rows(ctx) {
		...
		if pres {
			...
			err = copyUpload(ctx, config_obj, client_id, flow_id, hostname,
				password, zip_writer, path_spec)
			if err != nil {
				continue
			}
		}
...
func init() {
	vql_subsystem.OverrideFunction(&CreateHuntDownload{})
	vql_subsystem.OverrideFunction(&CreateFlowDownload{})
}
