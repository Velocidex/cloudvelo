...
func (self *Builder) OutputFile() string {
	return filepath.Join("output", self.Name())
}
...
func DockerImage() error {
	...
	fmt.Printf("Binary is at %v\n", builder.OutputFile())
	// Build the docker images. First copy the output binary into the
	// docker container.
	err = sh.Copy("./Docker/build/cvelociraptor", builder.OutputFile())
	if err != nil {
		return err
	}
	...
	os.Chdir("./Docker/build")

	return sh.RunWith(builder.Env(), "docker", "build", "-t", "cvelociraptor", "-f", "Dockerfile", ".")
}
